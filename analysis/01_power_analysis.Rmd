...
check_stopping_per_child <- function(fit,
  ...
  map_dfr(child_ids, function(cid) {
    ...
    if (!"NoPrep" %in% top_names) {
      ...
      cri_width <- as.numeric(quantile(diff_draws, 0.975) - quantile(diff_draws, 0.025))
...
```{r}
print_winners_list(adaptive_results[[1]]$child_status)
adaptive_results[[5]]$child_status[, c("true_profile", "true_winners", "best_condition", "p_best", "stop_session", "stop_reason", "detected")]
```
...
sim_data <- simulate_study_data(n_children = 500, n_sessions = 40, effect_size = 1.0)
