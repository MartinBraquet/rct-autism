...
- **Maximum:** 32 sessions, regardless of stopping status (derived from the 85% power simulation; see Appendix A).

To maintain developmental consistency and treatment density, data collection for any individual participant concludes after **8 weeks**, regardless of sessions completed. This gives a fair opportunity to children attending at least 3 sessions per week during the trial (>78% resolution rate, per the simulated power analysis). Data from participants who do not reach the 12-session minimum within this window are retained in the analysis following an Intent-to-Treat approach and will not be excluded.
...
**Baseline state on arrival:** Rated by the rater at session start as Tired/Lethargic, Neutral/Calm, or Hyperactive/Silly. Both Tired and Hyperactive states are expected to reduce engagement relative to Neutral. This covariate is excluded from the primary model to avoid overfitting (due to low sample size).
...
### 7.3 Pre-registered Decision Rule for Age
...
```  
Family: gaussian Formula: engagement ~ prep + teacher + age + (1 + prep || child_id)   
...
The maximum session count is set to achieve ≥ 85% power to resolve each child's optimal preparation condition, for a Smallest Effect Size of Interest (SESOI) of 2.0 points on the 10-point scale. A Monte Carlo adaptive simulation ($N_{\text{sim}} = 2000$) establishes this at **32 sessions** (see Appendix A for full details).

| Profile             | Resolved by 32 Sessions |
| ------------------- | ----------------------- |
| One winner — strong | 94%                     |
| One winner — weak   | 64%                     |
| Multiple winners    | 93%                     |
| No differential     | 57%                     |
| **Overall**         | **87%**                 |
...
To establish the 32-session maximum and validate the adaptive stopping rules, a Monte Carlo power analysis was conducted ($N_{\text{sim}} = 2000$ iterations).
...
The simulation confirms that at 32 sessions, the model achieves **87% overall power** to resolve a child's optimal preparation condition, with 94% power for children exhibiting strong response profiles.
...
See Figure 1 of the attached `registration_appendix.pdf` for the Power Analysis Resolution Curve.

Raw values:

| session | multiple_winners | no_differential | one_winner_strong | one_winner_weak | overall       |
| ------- | ---------------- | --------------- | ----------------- | --------------- | ------------- |
| 12      | 0.17142857       | 0.04            | 0.468085106382979 | 0.15625         | 0.21571428571 |
| 16      | 0.52619047       | 0.08            | 0.659574468085106 | 0.328125        | 0.48714285714 |
| 20      | 0.79285714       | 0.2533333       | 0.801418439716312 | 0.390625        | 0.7           |
| 24      | 0.87857142       | 0.3333333       | 0.858156028368794 | 0.515625        | 0.78285714285 |
| 28      | 0.90952380       | 0.4666666       | 0.907801418439716 | 0.59375         | 0.83285714285 |
| 32      | 0.93095238       | 0.5733333       | 0.943262411347518 | 0.640625        | 0.86857142857 |
| 36      | 0.95             | 0.64            | 0.957446808510638 | 0.734375        | 0.89857142857 |
